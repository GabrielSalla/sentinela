...
async def test_ntfynotification_create(monkeypatch, params):
    ...
    result = ntfy_notification.NtfyNotification.create(
        title="Test Monitor",
        params=params,
    )
...
async def test_ntfynotification_default_server_url_env(monkeypatch):
    ...
    monkeypatch.setenv("NTFY_SERVER_URL", "https://ntfy.example.com")
    result = ntfy_notification.NtfyNotification(topic="my-topic", title="Test Monitor")
...
async def test_build_message(
    ...
    """'_build_message' should join the event name, the alert priority and the issues count"""
    alert = await Alert.create(
        monitor_id=sample_monitor.id,
    ...
    result = await ntfy_notification._build_message(alert, "alert_created")

    assert "Event: alert_created" in result
    assert expected_priority in result
...
async def test_build_message_solved(sample_monitor: Monitor):
    ...
    result = await ntfy_notification._build_message(alert, "alert_solved")
...
async def test_handle_ntfy_notification_existing_resends(
    ...
    await Notification.create(
        monitor_id=alert.monitor_id, alert_id=alert.id, target="ntfy", data={"sent": True}
    )
    notification_options = ntfy_notification.NtfyNotification(
        topic="topic",
        title="title",
...
async def test_handle_ntfy_notification_alert_solved(mock_post_message, sample_monitor: Monitor):
    ...
    notification = await Notification.create(
        monitor_id=alert.monitor_id, alert_id=alert.id, target="ntfy"
    )
    notification_options = ntfy_notification.NtfyNotification(
        topic="topic",
        title="title",
