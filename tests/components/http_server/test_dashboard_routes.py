...
async def test_serve_css_asset():
...
async def test_serve_js_asset():
    """The dashboard should serve JavaScript assets correctly"""
    async with aiohttp.ClientSession() as session:
        async with session.get(BASE_URL + "/js/app.js") as response:
            assert response.status == 200
            assert response.content_type == "application/javascript"


async def test_serve_asset_not_found():
...
async def test_serve_asset_forbidden_path_traversal():
