...
async def test_create_user_forbidden(user_cookies):
    """The 'users create' route should reject non-admin users"""
    async with aiohttp.ClientSession(cookies=user_cookies) as session:
        async with session.post(
            BASE_URL + "/auth/users", json={"username": "forbidden_user"}
        ) as response:
            assert response.status == 403
            response_data = await response.json()
            assert response_data == {"status": "forbidden"}
...
async def test_recreate_invite(admin_cookies):
    """The 'users invite' route should return a fresh invite link"""
    cookies = admin_cookies

    async with aiohttp.ClientSession(cookies=cookies) as session:
        async with session.post(
            BASE_URL + "/auth/users", json={"username": "re_invite_user"}
        ) as response:
            assert response.status == 201
            response_data = await response.json()
            old_token = response_data["invite_url"].split("token=")[1]

        async with session.post(BASE_URL + "/auth/users/re_invite_user/invite") as response:
            assert response.status == 201
            response_data = await response.json()
            assert response_data["status"] == "invite_created"
            assert response_data["username"] == "re_invite_user"
            new_token = response_data["invite_url"].split("token=")[1]
            assert new_token != old_token

    async with aiohttp.ClientSession() as session:
        async with session.get(BASE_URL + f"/auth/invite/validate?token={old_token}") as response:
            assert response.status == 404

        async with session.get(BASE_URL + f"/auth/invite/validate?token={new_token}") as response:
            assert response.status == 200


async def test_recreate_invite_not_found(admin_cookies):
    """The 'users invite' route should return 404 for unknown users"""
    async with aiohttp.ClientSession(cookies=admin_cookies) as session:
        async with session.post(BASE_URL + "/auth/users/unknown_user/invite") as response:
            assert response.status == 404
            response_data = await response.json()
            assert response_data == {"status": "user_not_found"}


async def test_recreate_invite_password_set(admin_cookies, user_cookies):
    """The 'users invite' route should reject users that already have a password"""
    async with aiohttp.ClientSession(cookies=admin_cookies) as session:
        async with session.post(BASE_URL + "/auth/users/plain_user/invite") as response:
            assert response.status == 400
            response_data = await response.json()
            assert response_data["status"] == "password_already_set"


async def test_recreate_invite_forbidden(user_cookies):
    """The 'users invite' route should reject non-admin users"""
    async with aiohttp.ClientSession(cookies=user_cookies) as session:
        async with session.post(BASE_URL + "/auth/users/some_user/invite") as response:
            assert response.status == 403
            response_data = await response.json()
            assert response_data == {"status": "forbidden"}


async def test_recreate_invite_unauthorized():
    """The 'users invite' route should reject requests without a session"""
    async with aiohttp.ClientSession() as session:
        async with session.post(BASE_URL + "/auth/users/some_user/invite") as response:
            assert response.status == 401
            response_data = await response.json()
...
async def test_validate_invite(admin_cookies):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(BASE_URL + f"/auth/invite/validate?token={token}") as response:
            assert response.status == 200
            response_data = await response.json()
            assert response_data == {"status": "valid", "username": "invite_user"}
