...
async def test_ensure_default_admin_race(monkeypatch):
    """'ensure_default_admin' should return the existing admin when creation races"""
    existing = await service.ensure_default_admin()
    assert existing is not None
    gets = iter([None, existing])
    ...
    monkeypatch.setattr(User, "create", _create)
...
async def test_authenticate_disabled():
    """'authenticate' should reject disabled users"""
    admin = await _create_user("test_authenticate_disabled_admin", role=UserRole.admin)
    await _create_user("test_authenticate_disabled")
    await service.disable_user(admin, "test_authenticate_disabled")

    assert await service.authenticate("test_authenticate_disabled", STRONG_PASSWORD) is None
...
async def test_get_request_user_disabled():
    """'get_request_user' should return None for disabled users"""
    user = await _create_user("test_get_request_user_disabled")
    raw_token = service.create_session(user)
...
async def test_recreate_invite():
    """'recreate_invite' should return a fresh token and invalidate the previous one"""
    user, old_token = await service.create_user_with_invite("test_recreate_invite")

    _, new_token = await service.recreate_invite("test_recreate_invite")

    assert new_token != ""
    assert new_token != old_token
    with pytest.raises(InvalidInviteError):
        await service.validate_invite(old_token)
    found = await service.validate_invite(new_token)
    assert found.id == user.id


async def test_recreate_invite_not_found():
    """'recreate_invite' should reject unknown users"""
    with pytest.raises(UserNotFoundError):
        await service.recreate_invite("test_recreate_invite_missing")


async def test_recreate_invite_password_set():
    """'recreate_invite' should reject users that already have a password"""
    await _create_user("test_recreate_invite_password_set")

    with pytest.raises(PasswordAlreadySetError):
        await service.recreate_invite("test_recreate_invite_password_set")
