...
class Alert(Base):
    ...
    async def acknowledge(
        ...
        if send_event:
            ...
            await self._create_event("alert_acknowledged", extra_payload=extra_payload)
    ...
    async def lock(self, context: dict[str, Any] | None = None) -> None:
        ...
        extra_payload = {"context": context} if context is not None else None
        await self._create_event("alert_locked", extra_payload=extra_payload)
    ...
    async def update(self, context: dict[str, Any] | None = None) -> None:
        ...
        if issues_count == 0:
            ...
            extra_payload = {"context": context} if context is not None else None
            await self._create_event("alert_updated", extra_payload=extra_payload)
    ...
    async def solve(self, context: dict[str, Any] | None = None) -> None:
        ...
        extra_payload = {"context": context} if context is not None else None
        await self._create_event("alert_solved", extra_payload=extra_payload)
