...
async def disabled_command_message(command: str, context: dict[str, Any]) -> None:
    ...
    await slack.send(
        channel=channel,
        thread_ts=thread_ts,
        text=f"Command `{command}` is disabled",
    )


MENTION_PATTERN = r"(?:<@\w+>)? "
PATTERNS = {
    "alert_acknowledge": (r"?ack +(\d+)", alert_acknowledge),
    "alert_lock": (r"?lock +(\d+)", alert_lock),
    "alert_solve": (r"?solve +(\d+)", alert_solve),
    "issue_drop": (r"?drop issue +(\d+)", issue_drop),
    "monitor_disable": (r"?disable monitor +(\w+)", monitor_disable),
    "monitor_documentation": (r"?docs +(\w+)", monitor_documentation),
    "monitor_enable": (r"?enable monitor +(\w+)", monitor_enable),
    "monitor_refresh": (r"?refresh +(\w+)(?: +(search|update))?", monitor_refresh),
    "resend_notifications": (r"?resend notifications", resend_notifications),
}
...
def get_message_request(message: str, context: dict[str, Any]) -> Coroutine[Any, Any, Any] | None:
    ...
    for command, (pattern, get_action_function) in PATTERNS.items():
        match = re.match(MENTION_PATTERN + pattern, message)

        if match is None:
            continue

        if not commands_configs.get(command, {}).get("enabled", True):
            return disabled_command_message(command, context)
