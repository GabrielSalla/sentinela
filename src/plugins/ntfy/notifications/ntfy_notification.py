...
def _get_token(token_name: str) -> str:
    ...
    token = ntfy.tokens.get(token_name)
    if token is None:
        raise ValueError(
...
class NtfyNotification:
    ...
    Only the alert summary is sent (event name, priority, issues count and state), without the
    issues content, to keep the notification short.
    ...
    - `server_url`: The ntfy server URL. Uses the given value, else the `NTFY_SERVER_URL`
    environment variable, else `https://ntfy.sh`.
...
async def _build_message(alert: Alert, event_name: str) -> str:
    ...
    if alert.status == AlertStatus.solved:
        ...
        issues_count = await Issue.count(
            Issue.alert_id == alert.id, Issue.status == IssueStatus.active
        )
...
async def _handle_ntfy_notification(
    ...
    message = await _build_message(alert, event_name)
