...
# TODO: contornar problema de _monitors
# TODO: monitors internos não passam por validação dos imports
...
@contextmanager
def restrict_imports(base_path: str | None, module_name: str) -> Generator[None, None, None]:
    """Wrap the import functions into validators to block unauthorized module access. The
    restriction is only made at the module root level, ignoring imports done by deeper modules"""
    # Overwrite 'builtins.__import__'
    original_builtins_import = builtins.__import__

    module_path = get_module_path(base_path, module_name)

    def builtins_import_wrapper(*args, **kwargs) -> ModuleType:
        # Extract the module name: 'os.path' -> 'os'
        module_name_import = kwargs.get("name", args[0]).split(".")[0]
        is_module_root = inspect.stack()[1].filename.endswith(str(module_path))

        if is_module_root and module_name_import in PROHIBITED_IMPORTS:
