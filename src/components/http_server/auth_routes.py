...
@auth_routes.post(base_route + "/users/{username}/invite")
@auth_routes.post(base_route + "/users/{username}/invite/")
async def recreate_invite(request: Request) -> Response:
    """Route to recreate an invite token for a password-less user, admin only"""
    user: User = request[service.USER_REQUEST_KEY]
    if user.role != UserRole.admin:
        return web.json_response({"status": "forbidden"}, status=403)

    try:
        target, raw_token = await service.recreate_invite(request.match_info["username"])
    except UserNotFoundError:
        return web.json_response({"status": "user_not_found"}, status=404)
    except PasswordAlreadySetError as e:
        return web.json_response({"status": "password_already_set", "message": str(e)}, status=400)
