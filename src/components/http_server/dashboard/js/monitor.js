async function loadMonitors() {
    try {
        const response = await fetch(`${window.location.origin}/monitor/list`);
...
function updateMonitorSelect() {
    ...
    select.innerHTML = `
        <option value="" disabled hidden>Choose a monitor</option>
        <option value="___CREATE_NEW___" style="font-style: italic; color: #58a6ff;">Create new monitor</option>
        ${Object.values(state.monitors).map(m => `<option value="${m.name}">${m.name}</option>`).join('')}
    `;
    select.value = '';
}
