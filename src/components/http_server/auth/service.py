...
class PasswordAlreadySetError(ValueError):
    pass
...
async def recreate_invite(username: str) -> tuple[User, str]:
    """Create a fresh invite token for a password-less user, invalidating the previous one"""
    user = await User.get(User.username == username)
    if user is None:
        raise UserNotFoundError(f"User {username!r} not found")
    if user.password_hash is not None:
        raise PasswordAlreadySetError(f"User {username!r} already has a password")
